...
spool 'C:\Users\joand\OneDrive - University of Texas at Arlington\Fall '22 (Senior)\INSY 3304\Project 2' (the location)
...
FOREING KEY (CustID) REFERENCES PRODCAT_jds
);
...
ProdPrice	NUMBER(2,2)	NOT NULL,
PRIMARY KEY (ProdID),
FOREING KEY (ProdCatID) REFERENCES PRODCAT_jds
);


CREATE TABLE PRODCAT_jds(
ProdCatID	VARCHAR(3)	NOT NULL, 
...
SalesRepLName	NUMBER(1)	NOT NULL,
CommClass	NUMBER(2,2)	NOT NULL,
DeptID		NUMBER(2)	NOT NULL,
PRIMARY KEY (SalesRepID),
FOREING KEY (CommClass), REFERENCES COMMISSION_jds
FOREING KEY (DeptID) REFERENCES DEPARTMENT_jds
);
...
CustPhone	CHAR(10),
SalesRepID	NUMBER(3)	NO NULL,
PRIMARY KEY (CustID),
FOREING KEY (SalesRepID) REFERENCES SALESREP_jds
);
...
CustID		NUMBER(1)	NOT NULL,
PRIMARY KEY (OrderID),
FOREING KEY (CustID) REFERENCES CUSTOMER_jds
);
...
--INSERT rows into PRODUCT_jds (PRICES AND REP STUFF????)
...
--INSERT rows into CUSTOMER_jds (Phone # in blank???)
...
-- COMMIT the data to save it to disk (COMMIT applies only to INSERT, UPDATE, and 
DELETE statements)
COMMIT; 
